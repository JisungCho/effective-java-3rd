...
- 각 아이템 진행 순서: 개념 요약 → 예제 코드 → Learn by Doing(과제 + 검증용 테스트 동시 제공) → 학습자 구현·자가 검증 → md 정리 → 다음 아이템.
- 모든 예제 클래스는 `final` (상속 금지, Item 19와 정합).
- **Learn by Doing 과제 설계 원칙**:
  - TODO(human)은 단순 알고리즘/로직 구현이 아니라 **해당 아이템의 핵심 개념을 직접 코드로 체득하는 과제**여야 한다. (예: Item 4의 "private 생성자 + AssertionError 작성", Item 3의 "싱글턴 상태 누적 관찰")
  - **과제 제시 시 검증용 테스트 코드를 함께 제공**한다. 학습자가 스스로 피드백 루프를 닫을 수 있게. 테스트는 흔한 실수를 정확히 잡아내도록 설계. (Item 7 학습 중 학습자 요청, 2026-08-03)
...
- [x] Item 7: 다 쓴 객체 참조를 해제하라 (ch02/item07) — 완료
- [x] Item 8: finalizer와 cleaner 사용을 피하라 (ch02/item08) — 완료
- [ ] Item 9 ~ 90: 미진행
